...
Shader "Skybox/Procedural TOON GIBLI" {
	Properties{
		...
		_UpVectorYaw("Up Vector Yaw", float) = 0
		  ...
		  _Exponent1("Exponent Factor for Top Half", Float) = 1.0
		   ...
		   _IntensityB("Intensity Amplifier", Float) = 1.0

		_ControlVector("Control Vector", Vector) = (1, 1, 1, 1)
		  posterizeFactor("posterizeFactor", Float) = 1.0
			posterizeColorNumber("posterizeColorNumber", Float) = 10.0
		darkenSun("darkenSun",Vector) = (0, 0, 1, 1)
	}
		...
		SubShader{
			...
			//v0.1
			float4 _ControlVector;
			float  posterizeFactor;
			float posterizeColorNumber;
			float4 darkenSun;
...
//              for(int i=0; i<int(kSamples); i++) // Loop removed because we kept hitting SM2.0 temp variable limits. Doesn't affect the image too much.
        ...
        // if y < 0 [eyeRay.y > 0] - sky
			...
			//https://github.com/keijiro/UnitySkyboxShaders MIT
			half d = dot(normalize(IN.texcoord), _UpVector) * 0.5f + 0.5f;
			float4 outA = lerp(_Color1, _Color2, pow(d, _Exponent)) * _Intensity;

			float p = normalize(IN.texcoord).y;
			float p1 = 1.0f - pow(min(1.0f, 1.0f - p), _Exponent1);
			float p3 = 1.0f - pow(min(1.0f, 1.0f + p), _Exponent2);
			float p2 = 1.0f - p1 - p3;
			float4 outB = (_Color1 * p1 + _Color2 * p2 + _Color3 * p3) * _IntensityB;



			//v0.1
            //return (half4(col,1.0));
			col = darkenSun.x * col*0.5*_ControlVector.x + (1 - darkenSun.x) * saturate(col*0.5*_ControlVector.x);
			float4 posterIN = (half4(col, 1.0)) + outA*0.25*_ControlVector.y + outB*0.25*_ControlVector.z;

			if (posterizeFactor > 0) {
				posterIN.rgb = posterizeColorNumber * pow(posterIN.rgb, posterizeFactor);
				posterIN.rgb = floor(posterIN.rgb) * (1.0 / posterizeColorNumber);
				posterIN.rgb = pow(posterIN.rgb, (1.0 / posterizeFactor));
			}

			return saturate(posterIN) *0.95*darkenSun.y + (posterIN) *0.05*darkenSun.z;
        }
        ENDCG
    }
}
