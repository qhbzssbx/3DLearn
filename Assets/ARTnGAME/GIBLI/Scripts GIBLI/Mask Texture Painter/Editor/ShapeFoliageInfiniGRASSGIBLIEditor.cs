...
{
    [CustomEditor(typeof(ShapeFoliageInfiniGRASSGIBLI))]
    public class ShapeFoliageInfiniGRASSGIBLIEditor : Editor
    {
        //v1.2
        ...
        private ShapeFoliageInfiniGRASSGIBLI script;
        void Awake()
        {
            script = (ShapeFoliageInfiniGRASSGIBLI)target;
        }
        ...
        void PaintOnManager(ShapeFoliageInfiniGRASSGIBLI globalManager, Event cur)//void PaintOnManager(InfiniGRASSManager script, GlobalGrassManager globalManager, Event cur)
        {
            ...
            if (script.Grass_painting & !Application.isPlaying)
            {
                ...
                else
                {
                    ...
                    if (!looking && cur.keyCode != (KeyCode.LeftControl) && cur.keyCode != (KeyCode.LeftAlt) && cur.type != EventType.KeyDown &&
                        ((cur.type == EventType.MouseDrag && buttonPressed && Vector3.Distance(Keep_last_mouse_pos, cur.mousePosition) > script.min_grass_patch_dist)//script.min_grass_patch_dist)
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (1 == 1)//if ((script.UseTabs && script.currentTab == 2) | !script.UseTabs)
            {
                ...
                //if (script.massEraseFolder)
                {
                    ...
                    //v1.2
                    EditorGUILayout.PropertyField(GrassMaterials); 
