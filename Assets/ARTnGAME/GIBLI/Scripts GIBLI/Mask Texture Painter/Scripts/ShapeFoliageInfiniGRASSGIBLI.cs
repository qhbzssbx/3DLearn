...
namespace Artngame.GIBLI {
    ...
    //v2.1.1 - Interactors
    [System.Serializable]
    public class InfiniGRASS_InteractorsGIBLI
    {
    ...
    public class ShapeFoliageInfiniGRASSGIBLI : MonoBehaviour
    {
        public float min_grass_patch_dist = 1;
        ...
        public List<InfiniGRASS_InteractorsGIBLI> Interactors = new List<InfiniGRASS_InteractorsGIBLI>();
        ...
        { //v2.1.8 //v2.1.10
            ...
            if (TextureInteract && Interactors.Count > 0)
            {
                for (int j = 0; j < Interactors.Count; j++)
                {
                    ...
                    InfiniGRASSInteractorGIBLI interactorProps = Interactors[j].Interactor.GetComponent<InfiniGRASSInteractorGIBLI>();
        ...
        //v2.1.28
        void plantOnRaycast(ShapeFoliageInfiniGRASSGIBLI globalManager, RaycastHit hit, Ray ray, bool erasing)
        {
        ...
        public void AssignShapeTextureToMaterials(Texture2D InternalInteractTexture)
        {
            if (InternalInteractTexture != null)
            {
                ...
                {
                    if (GrassMaterials[i].HasProperty("_InteractTextureA"))
                    {
                        GrassMaterials[i].SetTexture("_InteractTextureA", InternalInteractTexture);

                        //Assign tex sizing
                        GrassMaterials[i].SetVector("_InteractTexturePosA", new Vector3(0, InternalInteractTexture.width / textureScale, 0));
                    }
                }
