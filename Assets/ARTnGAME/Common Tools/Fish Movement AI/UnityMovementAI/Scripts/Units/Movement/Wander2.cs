...
namespace Artngame.RiverStudio.UnityMovementAI
{
    ...
    public class Wander2 : MonoBehaviour
    {
        ...
        void Start()
        {
            ...
            if (rb.is3D)
            {
                wanderTarget = new Vector3(wanderRadius * Mathf.Cos(theta), Random.Range(-1f, 1f)  * Ydisp, wanderRadius * Mathf.Sin(theta));
            }
            else
            {
        ...
        public Vector3 GetSteering()
        {
            ...
            if (rb.is3D)
            {
                wanderTarget += new Vector3(Random.Range(-1f, 1f) * jitter, Random.Range(-1f, 1f) * jitter * Ydisp, Random.Range(-1f, 1f) * jitter);
            }
            else
            {
